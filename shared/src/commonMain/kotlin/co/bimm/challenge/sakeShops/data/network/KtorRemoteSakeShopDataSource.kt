...
import co.bimm.challenge.core.data.safeCall
...
class KtorRemoteSakeShopDataSource(
    private val httpClient: HttpClient,
    private val useMockData: Boolean = true
) : RemoteSakeShopDataSource {
    ...
    override suspend fun searchSakesShops(resultLimit: Int?): Result<SakeShopResponse, DataError.Remote> {
        return if (useMockData) {
            getMockSakeShopResponse(resultLimit)
        } else {
            safeCall<SakeShopResponse> {
                httpClient.get(
    ...
    private suspend fun getMockSakeShopResponse(resultLimit: Int?): Result<SakeShopResponse, DataError.Remote> {
        return try {
            delay(500L)
            val sakeShops = json.decodeFromString<List<SakeShop>>(getMockJsonData())
            val limitedShops = resultLimit?.let { limit ->
                sakeShops.take(limit)
            } ?: sakeShops
            val response = SakeShopResponse(data = limitedShops)
    ...
    private fun getMockJsonData(): String = """
        [
            {
                "name": "信州スシサカバ 寿しなの",
